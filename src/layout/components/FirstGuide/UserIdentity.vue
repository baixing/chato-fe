<template>
  <div class="w-full userDusty-container">
    <el-form label-position="top" :rules="rules" :model="form">
        ...
        <div class="flex justify-between items-center w-full">
          <div
            ...
            :class="[
              ...
              currentIdentity.name === item.name ? 'border-[#7C5CFC] text-[#7C5CFC]' : ''
            ]"
            :key="item.id"
      ...
      <el-form-item :label="$t(currentIdentity.label)" prop="input">
        <el-input
          size="large"
          v-model="form.input"
          class="w-full"
...
const currentIdentity = ref(identityList[0])
const rules = reactive({
  input: [{ required: true, message: t(currentIdentity.value.placeholder), trigger: 'blur' }]
})
const form = reactive({
  input: ''
})

watch([() => form.input, () => currentIdentity.value.id], () => {
  const userIndentityResult = {
    origanization_type: currentIdentity.value.id,
    company: '',
    surname: ''
  }
...
watch(
  ...
  (v) => {
    const index = identityList.findIndex((item) => item.id === v.origanization_type)
    form.input = v.company || v.surname || ''
    currentIdentity.value = index !== -1 ? identityList[index] : identityList[0]
  },
  { immediate: true }
)
</script>
